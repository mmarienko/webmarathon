...
class Users extends Model
{
    ...
    public function login($login, $password)
    {
        if ($this->database->getConnectionStatus()) {
            try {
                $sth = $this->database->connection->prepare("SELECT login, password, status FROM users WHERE login = :login AND password = :password");
    ...
    public function create($login, $password, $fullname, $email, $status)
    {
        if ($this->database->getConnectionStatus()) {
            try {
